...
class S3DIS_base(Dataset):
    def __init__(
        ...
        self.class_count = 13
        dataset_root = os.path.dirname(os.path.dirname(self.data_root))
        classnames_path = os.path.join(dataset_root, "meta", "s3dis_classnames.txt")
        if not os.path.exists(classnames_path):
            classnames_path = os.path.join(
        ...
        if not os.path.exists(classnames_path):
            raise FileNotFoundError(
                f"Cannot find s3dis_classnames.txt. Tried: {classnames_path}"
            )
        class_names = open(classnames_path).readlines()
        self.class2type = {
